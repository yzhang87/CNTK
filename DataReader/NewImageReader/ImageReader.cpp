...
namespace Microsoft { namespace MSR { namespace CNTK {
        ...
        , m_elementType(elementType)
    {
        ...
        TransformerPtr cropper = std::make_shared<CropTransform>(randomizer, m_inputs, config, m_seed);
        TransformerPtr scaler = std::make_shared<ScaleTransform>(cropper, m_inputs, config, m_seed, m_elementType == et_float ? CV_32F : CV_64F);
