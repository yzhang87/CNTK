...
namespace Microsoft {
    namespace MSR {
        namespace CNTK {
            ...
            NormalPacker<ElemType>::NormalPacker() : m_pMBLayout(make_shared<MBLayout>())
            {
            }
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::Init(const ConfigParameters& readerConfig, shared_ptr<IMemoryProvider> Provider)
            {
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::StartDistributedMinibatchLoop(size_t requestedMBSize, size_t epoch, size_t subsetNum, size_t numSubsets, size_t requestedEpochSamples /*= requestDataSize*/)
            {
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::StartMinibatchLoopToTrainOrTest(size_t mbSize, size_t epoch, size_t subsetNum, size_t numSubsets, size_t requestedEpochSamples)
            {
                size_t datapasses = 1;
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::StartMinibatchLoopToWrite(size_t mbSize, size_t /*epoch*/, size_t /*requestedEpochSamples*/)
            {
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetMinibatch4SE(std::vector<shared_ptr< const msra::dbn::latticesource::latticepair>> & latticeinput,
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetMinibatch4SEToTrainOrTest(std::vector<shared_ptr<const msra::dbn::latticesource::latticepair>> & latticeinput,
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetHmmData(msra::asr::simplesenonehmm * hmm)
            {
                *hmm = m_hset;
                return true;
            }
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetMinibatch(std::map<std::wstring, Matrix<ElemType>*>& matrices, MBLayoutPtr returnLayout)
            {
                if (m_trainOrTest)
                {
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetMinibatchToTrainOrTest(std::map<std::wstring, Matrix<ElemType>*>& matrices)
            {
                size_t id;
                size_t dim;
                bool skip = false;
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::fillOneUttDataforParallelmode(std::map<std::wstring, Matrix<ElemType>*>& matrices, size_t startFr,
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::GetMinibatchToWrite(std::map<std::wstring, Matrix<ElemType>*>& matrices)
            {
                std::map<std::wstring, size_t>::iterator iter;
                if (m_checkDictionaryKeys)
                {
            ...
            template<class ElemType>
            bool NormalPacker<ElemType>::ReNewBufferForMultiIO(size_t i)
            {
                if (m_noData)
                {
                    if ((i == 0) && !m_truncated)
            ...
            template<class ElemType>
            size_t NormalPacker<ElemType>::ReadLabelToTargetMappingFile(const std::wstring& labelToTargetMappingFile, const std::wstring& labelListFile, std::vector<std::vector<ElemType>>& labelToTargetMap)
            {
                if (labelListFile == L"")
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::SetSentenceEndInBatch(vector<size_t> &sentenceEnd)
            {
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::CopyMBLayoutTo(MBLayoutPtr pMBLayout)
            {
                pMBLayout->CopyFrom(m_pMBLayout);
            }

            template<class ElemType>
            size_t NormalPacker<ElemType>::GetNumParallelSequences()
            {
                if (!m_frameMode)
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::GetDataNamesFromConfig(const ConfigParameters& readerConfig, std::vector<std::wstring>& features, std::vector<std::wstring>& labels,
            ...
            template<class ElemType>
            void NormalPacker<ElemType>::ExpandDotDotDot(wstring & featPath, const wstring & scpPath, wstring & scpDirCached)
            {
                wstring delim = L"/\\";
            ...
            template<class ElemType>
            std::shared_ptr<ElemType> NormalPacker<ElemType>::Allocate(size_t numberOfElements)
            {
            ...
            minibatch NormalPacker<ElemType>::epoch_impl::read_minibatch()
            {
                ...
                std::map<std::wstring, Matrix<ElemType>*> m;
                m.insert(std::make_pair(std::wstring(L"features"), &features));
                m.insert(std::make_pair(std::wstring(L"labels"), &labels));

                MBLayoutPtr layout(new MBLayout);
                bool result = packer_->GetMinibatch(m, layout);
                if (!result)
                {
                    return minibatch();
                }
                ...
                {
                    auto elements = features.GetNumElements();
                    auto buf = new char[elements * sizeof(ElemType)];
                    auto tmp = features.CopyToArray();
                    memcpy(buf, tmp, elements * sizeof(ElemType));

                    minibatch r;
                    // should be taken from mblayout
                    input_ptr i1(new input(buf, elements * sizeof(ElemType), layout_ptr()));
                    r.mb.insert(std::make_pair(0, i1));

                    std::unique_ptr<std::map<size_t, input_ptr>> r2(new std::map<size_t, input_ptr>);
                    // cause a crash :)
                    input_ptr i2(new input(buf, elements * sizeof(ElemType), layout_ptr()));
                    r.mb.insert(std::make_pair(0, i2));
                    return r;
                }
            }
