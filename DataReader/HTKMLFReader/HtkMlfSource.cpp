...
namespace Microsoft {
    namespace MSR {
        namespace CNTK {
            ...
            template<class ElemType>
            void HTKMLFSource<ElemType>::PrepareForTrainingOrTesting(const ConfigParameters& readerConfig)
            {
                ...
                //std::vector<msra::asr::HTKMLFSource<msra::asr::htkmlfentry,msra::lattices::lattice::htkmlfwordsequence>> labelsmulti;
            ...
            // are inputs or targets for the network
            template<class ElemType>
            void HTKMLFSource<ElemType>::PrepareForWriting(const ConfigParameters& readerConfig)
            {
                vector<wstring> scriptpaths;
                size_t numFiles;
                size_t firstfilesonly = SIZE_MAX;   // set to a lower value for testing
                ...
                foreach_index(i, scriptpaths)
                {
                    ...
                    if (i == 0)
                        ...
                        if (n != numFiles)
                            RuntimeError(msra::strfun::strprintf("HTKMLFSource::InitEvalReader: number of files in each scriptfile inconsistent (%d vs. %d)", numFiles, n));
            ...
            template<class ElemType>
            void HTKMLFSource<ElemType>::StartMinibatchLoopToWrite(size_t mbSize, size_t /*epoch*/, size_t /*requestedEpochSamples*/)
            {
            ...
            template<class ElemType>
            bool HTKMLFSource<ElemType>::GetMinibatch4SE(std::vector<shared_ptr< const msra::dbn::latticesource::latticepair>> & latticeinput,
            ...
            template<class ElemType>
            bool HTKMLFSource<ElemType>::GetMinibatch(std::map<std::wstring, Matrix<ElemType>*>& matrices, MBLayoutPtr returnLayout)
            {
                if (m_trainOrTest)
                {
            ...
            template<class ElemType>
            bool HTKMLFSource<ElemType>::GetMinibatchToTrainOrTest(std::map<std::wstring, Matrix<ElemType>*>& matrices)
            {
                size_t id;
                size_t dim;
                bool skip = false;
                            ...
                            if ((m_processedFrame[i] + m_mbNumTimeSteps) < m_numFramesToProcess[i])
                            {
                                ...
                                for (iter = matrices.begin(); iter != matrices.end(); iter++)
                                {
                                    ...
                                    if (m_nameToTypeMap[iter->first] == InputOutputTypes::real)
                                    {
                                        ...
                                        {
                                            // ELDAK m_featuresBufferMultiIO[id] = Allocate(dim * m_mbNumTimeSteps * m_numSeqsPerMB);
                                    ...
                                    else if (m_nameToTypeMap[iter->first] == InputOutputTypes::category)
                                    {
                                        ...
                                        {
                                            // ELDAK m_labelsBufferMultiIO[id] = Allocate(dim * m_mbNumTimeSteps * m_numSeqsPerMB);
                                            m_labelsBufferAllocatedMultiIO[id] = dim * m_mbNumTimeSteps * m_numSeqsPerMB;
                                        }
            ...
            template<class ElemType>
            void HTKMLFSource<ElemType>::fillOneUttDataforParallelmode(std::map<std::wstring, Matrix<ElemType>*>& matrices, size_t startFr,
            ...
            template<class ElemType>
            bool HTKMLFSource<ElemType>::GetMinibatchToWrite(std::map<std::wstring, Matrix<ElemType>*>& matrices)
            {
                std::map<std::wstring, size_t>::iterator iter;
                if (m_checkDictionaryKeys)
                {
                ...
                if (m_inputFileIndex<m_inputFilesMultiIO[0].size())
                {
                    ...
                    for (iter = matrices.begin(); iter != matrices.end(); iter++)
                    {
                        ...
                        if (m_nameToTypeMap.find(iter->first) != m_nameToTypeMap.end() && m_nameToTypeMap[iter->first] == InputOutputTypes::real)
                        {
                            ...
                            {
                                // ELDAK m_featuresBufferMultiIO[id] = Allocate(feat.rows() * feat.cols());
            ...
            template<class ElemType>
            bool HTKMLFSource<ElemType>::ReNewBufferForMultiIO(size_t i)
            {
                if (m_noData)
                {
                    if ((i == 0) && !m_truncated)
                ...
                {
                    // ELDAK m_featuresBufferMultiUtt[i] = Allocate(totalFeatNum);
                ...
                {
                    // ELDAK m_labelsBufferMultiUtt[i] = Allocate(totalLabelsNum);
            ...
            {
                if (labelListFile == L"")
                    RuntimeError("HTKMLFSource::ReadLabelToTargetMappingFile(): cannot read labelToTargetMappingFile without a labelMappingFile!");
                ...
                while (mapReader)
                {
                    ...
                    if (labelList[count] != token)
                        RuntimeError("HTKMLFSource::ReadLabelToTargetMappingFile(): mismatch between labelMappingFile and labelToTargetMappingFile");

                    if (count == 0)
                        targetDim = targetstring.length();
                    else if (targetDim != targetstring.length())
                        RuntimeError("HTKMLFSource::ReadLabelToTargetMappingFile(): inconsistent target length among records");
                    ...
                    foreach_index(i, targetstring)
                    {
                        if (targetstring.compare(i, 1, L"1") == 0)
                            ...
                            RuntimeError("HTKMLFSource::ReadLabelToTargetMappingFile(): expecting label2target mapping to contain only 1's or 0's");
                    }
                ...
                if (count != labelList.size())
                    RuntimeError("HTKMLFSource::ReadLabelToTargetMappingFile(): mismatch between lengths of labelMappingFile vs labelToTargetMappingFile");

                return targetDim;
            }

            template<class ElemType>
            void HTKMLFSource<ElemType>::SetSentenceEndInBatch(vector<size_t> &sentenceEnd)
            {
            ...
            template<class ElemType>
            void HTKMLFSource<ElemType>::GetDataNamesFromConfig(const ConfigParameters& readerConfig, std::vector<std::wstring>& features, std::vector<std::wstring>& labels,
            ...
            template<class ElemType>
            void HTKMLFSource<ElemType>::ExpandDotDotDot(wstring & featPath, const wstring & scpPath, wstring & scpDirCached)
            {
                wstring delim = L"/\\";
