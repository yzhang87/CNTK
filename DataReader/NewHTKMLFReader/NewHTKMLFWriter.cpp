//
// <copyright file="NewHTKMLFWriter.cpp" company="Microsoft">
...
namespace Microsoft { namespace MSR { namespace CNTK {
    ...
    void NewHTKMLFWriter<ElemType>::InitFromConfig(const ConfigRecordType & writerConfig)
    {
        m_tempArray = nullptr;
        m_tempArraySize = 0;
        ...
        foreach_index(i, outputNames) // inputNames should map to node names
        {
            ...
            if (thisOutput.Exists("dim"))
                ...
                RuntimeError("NewHTKMLFWriter::Init: writer need to specify dim of output");
            ...
            if (thisOutput.Exists("file"))
                ...
                RuntimeError("NewHTKMLFWriter::Init: writer needs to specify scpFile for output");
            ...
            {
                RuntimeError("NewHTKMLFWriter::Init: output type for writer output expected to be Real");
            }
        }
        ...
        foreach_index(i,scriptpaths)
        {
            ...
            fprintf(stderr, "NewHTKMLFWriter::Init: reading output script file %ls ...", scriptPath.c_str());
            size_t n = 0;
            ...
            if (i==0)
                ...
                if (n!=numFiles)
                    RuntimeError("NewHTKMLFWriter:Init: number of files in each scriptfile inconsistent (%d vs. %d)", (int)numFiles, (int)n);
    ...
    template<class ElemType>
    void NewHTKMLFWriter<ElemType>::Destroy()
    {
        delete [] m_tempArray;
        m_tempArray = nullptr;
        m_tempArraySize = 0;
    ...
    template<class ElemType>
    void NewHTKMLFWriter<ElemType>::GetSections(std::map<std::wstring, SectionType, nocase_compare>& /*sections*/)
    {
    }

    template<class ElemType>
    bool NewHTKMLFWriter<ElemType>::SaveData(size_t /*recordStart*/, const std::map<std::wstring, void*, nocase_compare>& matrices, size_t /*numRecords*/, size_t /*datasetSize*/, size_t /*byteVariableSized*/)
    {
    ...
    template<class ElemType>
    void NewHTKMLFWriter<ElemType>::SaveToFile(std::wstring& outputFile, Matrix<ElemType>& outputData)
    {
    ...
    template<class ElemType>
    void NewHTKMLFWriter<ElemType>::SaveMapping(std::wstring saveId, const std::map<LabelIdType, LabelType>& /*labelMapping*/)
    {
    }
   
    template class NewHTKMLFWriter<float>;
    template class NewHTKMLFWriter<double>;
