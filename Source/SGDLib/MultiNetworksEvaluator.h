...
{
    typedef SimpleEvaluator<ElemType> Base;
    using Base::m_net;
        ...
        for (itdx = 0; itdx < maxSize; itdx++)
        {
            ...
            while (!from_queue.empty())
            {
                ...
                for (int i = 0; i < evalNodes.size(); i++)
                {
                    ...
                    if (GetCandidatesAtOneTimeInstance(dynamic_pointer_cast<ComputationNode<ElemType>>(evalNodes[i])->Value(), from_token.score, best_score - beam, -numeric_limits<double>::infinity(), retPair) == false)
                        continue;
            ...
            while (!from_queue.empty())
            {
                ...
                for (int i = 0; i < evalNodes.size(); i++)
                {
                    ...
                    if (GetCandidatesAtOneTimeInstance(dynamic_pointer_cast<ComputationNode<ElemType>>(evalNodes[i])->Value(),
                                                       from_token.score, best_score - beam, -numeric_limits<double>::infinity(), retPair) == false) // ==false??? !(.)?
                        continue;
