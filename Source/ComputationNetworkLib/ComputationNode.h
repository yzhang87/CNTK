...
public:
    ...
    ComputationNodeBase(DEVICEID_TYPE deviceId, const wstring& name)
        : m_deviceId(deviceId), m_outputNeededDuringBackprop(true), m_parameterUpdateRequired(false), m_gradientInitialized(false), m_nodeName(name == L"" ? CreateUniqNodeName() : name), m_numRows(0), m_numCols(0)
    {
    }
    ...
    {
        assert(m_numRows == m_sampleLayout.GetAllocation());
        return m_numRows;
    }
    ...
    pair<size_t, size_t> GetDims()
    {
        return make_pair(GetNumRows(), GetNumCols());
    }
    ...
    {
    } // someone outside changed our m_value--update our internal state, e.g. m_numRows, m_numCols
